    ...
    <div className="view-container fade-in">
      ...
      {appointments === null ? (
        ...
        <div className="dashboard-card" style={{ marginTop: '20px', textAlign: 'center', padding: '48px 24px' }}>
          ...
          <h3 style={{ margin: '12px 0 6px' }}>{t(language, 'health.emptyTitle')}</h3>
          <p style={{ color: '#64748b', fontSize: '0.9rem', marginBottom: '18px' }}>
            {t(language, 'health.emptyDesc')}
          </p>
          <button
            onClick={onTalk}
            style={{ padding: '10px 22px', borderRadius: '8px', border: 'none', background: '#3b82f6', color: 'white', fontWeight: 600, cursor: 'pointer' }}
          >
            {t(language, 'health.talk')}
          </button>
        </div>
      ) : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '16px', marginTop: '20px' }}>
            {stat(t(language, 'health.total'), all.length, '#2563eb')}
            {stat(t(language, 'health.upcoming'), upcoming.length, '#16a34a')}
            {stat(t(language, 'health.active'), completedOrActive, '#0891b2')}
            {stat(t(language, 'health.cancelled'), cancelled, '#ef4444')}
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '16px' }}>
            <div className="dashboard-card">
              <h3 style={{ marginBottom: '12px' }}>{t(language, 'health.next')}</h3>
              {next ? (
            ...
            <div className="dashboard-card">
              <h3 style={{ marginBottom: '12px' }}>{t(language, 'health.regular')}</h3>
              {favourite ? (
                <div style={{ fontSize: '0.95rem', color: '#334155' }}>
                  <strong>{favourite[0]}</strong>
                  <div style={{ color: '#64748b', marginTop: '6px' }}>{t(language, 'health.visitsSoFar', { n: favourite[1] })}</div>
                </div>
              ) : (
                <p style={{ color: '#64748b', fontSize: '0.9rem' }}>{t(language, 'health.noVisits')}</p>
              )}
            </div>
          </div>
        </>
